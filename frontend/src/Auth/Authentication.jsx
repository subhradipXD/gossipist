...
const Authentication = () => {
    ...
    const handleLogIn = async () => {
        setLoading(true);
        try {
            const userCredential = await signInWithEmailAndPassword(auth, lEmail, lPass);
                ...
                <div className="w-1/2 flex flex-col items-center justify-center border-r border-r-indigo-500 p-10">
                    <div className="mr-10 flex flex-col items-center space-y-2">
                        ...
                        <div className="flex flex-col mt-5 space-y-2">
                            <label className="text-lg font-semibold text-indigo-500">Username</label>
                            <input
                                type="text"
                            ...
                            <button
                                type="submit"
                                className={`mt-4 p-1 rounded-md text-white transition duration-300 ${loading ? 'bg-gray-400 cursor-not-allowed' : 'bg-indigo-500 hover:bg-indigo-700'}`}
                                onClick={() => {
                                    if (!loading) {
                                        handleSignUp();
                                        setLEmail("");
